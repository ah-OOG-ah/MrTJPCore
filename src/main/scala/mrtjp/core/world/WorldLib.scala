...
object WorldLib {
  ...
  def getTileEntity[T](
    ...
    if (y < 0) null.asInstanceOf[T]
      ...
      if (!clazz.isInstance(tile)) null.asInstanceOf[T]
      else tile.asInstanceOf[T]
    }
  }
  ...
  def bulkBlockUpdate(w: World, x: Int, y: Int, z: Int, bl: Block) {
    for (a <- -3 to 3) for (b <- -3 to 3) for (c <- -3 to 3) {
      val md =
        (if (a < 0) -a else a) + (if (b < 0) -b else b) + (if (c < 0) -c else c)
      if (md <= 3) {
        val block = w.getBlock(x + a, y + b, z + c)
        if (block != null)
          block.onNeighborBlockChange(w, x + a, y + b, z + c, bl)
      }
    }
  }
  ...
  def uncheckedSetBlock(
    ...
    if (arr(y >> 4) == null)
      arr(y >> 4) =
        new ExtendedBlockStorage(y & (~0xf), !world.provider.hasNoSky)
  ...
  def uncheckedGetTileEntity(world: World, x: Int, y: Int, z: Int) = {
    val ch = world.getChunkFromBlockCoords(x, z)
    ch.chunkTileEntityMap
      .get(new ChunkPosition(x & 0xf, y, z & 0xf))
      .asInstanceOf[TileEntity]
  }
  ...
  def isLeafType(world: World, x: Int, y: Int, z: Int, b: Block) =
    b.isLeaves(world, x, y, z) || OreDictionary
      .getOreIDs(new ItemStack(b))
      .contains(OreDictionary.getOreID("treeLeaves"))
  ...
  def isAssociatedTreeBlock(world: World, x: Int, y: Int, z: Int, b: Block) = {
    ...
    Seq(log, log2, leaves, leaves2, vine, cocoa)
      .contains(b) || isLeafType(world, x, y, z, b) ||
    OreDictionary
      .getOreIDs(new ItemStack(b))
      .contains(OreDictionary.getOreID("logWood"))
  }
  ...
  def findSurfaceHeight(world: World, x: Int, z: Int) = {
    ...
    do y -= 1 while (y >= 0 && {
      ...
      isBlockSoft(world, x, y, z, b) || isAssociatedTreeBlock(world, x, y, z, b)
    })
    y
  }
  ...
  def isBlockUnderTree(world: World, x: Int, y: Int, z: Int): Boolean = {
    ...
    for (h <- y until world.getHeight) {
      ...
      if (
        isLeafType(world, x, h, z, b) || isAssociatedTreeBlock(
          world,
          x,
          h,
          z,
  ...
  def getBlockLightValue(w: World, x: Int, y: Int, z: Int) =
  ...
  def getWindSpeed(w: World, x: Int, y: Int, z: Int): Double = {
    ...
    var nv =
      noise.noise(w.getWorldTime * 0.00000085d, 0, 0, 5, 7.5d, 5.0d, true)
