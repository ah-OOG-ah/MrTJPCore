...
object SimpleGenHandler extends IWorldGenerator {
  ...
  private var genQueue =
    Map[Int, MQueue[ChunkCoord]]().withDefaultValue(MQueue[ChunkCoord]())
  ...
  def chunkLoadEvent(event: ChunkDataEvent.Load) {
    if (retroGen) {
      ...
      val list =
        if (tag == null) new NBTTagList
        else tag.getTagList("StructList", Constants.NBT.TAG_STRING)

      if (
        tag == null || tag.getLong(
          "StructHash"
        ) != structHash || list.tagCount != structures.size
      ) {
        val chunk =
          ChunkCoord(event.getChunk.xPosition, event.getChunk.zPosition, list)
  ...
  private def subGenerate(
      w: World,
      chunkX: Int,
      chunkZ: Int,
      rand: Random,
      isRetro: Boolean,
      existingStructs: Set[String] = Set.empty
  ) {
    var gen = false
    for (s <- structures)
      if (!existingStructs.contains(s.genID))
  ...
  def tickEnd(event: WorldTickEvent) {
    ...
    if (chunks.nonEmpty) {
      ...
      log.warn(
        "Starting Retrogeneration on chunk @[DIM " + dim + "]" + chunk.toString
      )
      subGenerate(
        world,
        chunk.chunkX,
