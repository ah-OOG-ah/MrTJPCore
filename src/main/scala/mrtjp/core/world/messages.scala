...
object Messenger {
  ...
  def createPacket =
    new PacketCustom(MrTJPCoreSPH.channel, MrTJPCoreSPH.messagePacket)
  ...
  def addMessage(x: Double, y: Double, z: Double, mail: String) {
    val location = new BlockCoord(
      Math.floor(x).asInstanceOf[Int],
      Math.floor(y).asInstanceOf[Int],
      Math.floor(z).asInstanceOf[Int]
    )
  ...
  def renderMessages(event: RenderWorldLastEvent) {
    ...
    val cx =
      view.lastTickPosX + (view.posX - view.lastTickPosX) * event.partialTicks
    val cy =
      view.lastTickPosY + (view.posY - view.lastTickPosY) * event.partialTicks
    val cz =
      view.lastTickPosZ + (view.posZ - view.lastTickPosZ) * event.partialTicks
  ...
  private def readMessage(m: Message) {
    ...
    scaling *= 0.6666667f
    val y = (m.y + 0.04 * Math.sin(
      (m.x.asInstanceOf[Int] ^ m.z
        .asInstanceOf[Int]) + RenderTicker.getRenderTime / 4
    ) + m.yOffset).asInstanceOf[Float]

    GL11.glPushMatrix()
    GL11.glTranslated(m.x + 0.5f, y, m.z + 0.5f)
    GL11.glNormal3f(0.0f, 1.0f, 0.0f)
    GL11.glRotatef(
      (-RenderManager.instance.playerViewY + 8 * Math.sin(
        (m.x.asInstanceOf[Int] ^ m.z
          .asInstanceOf[Int]) + RenderTicker.getRenderTime / 6
      )).asInstanceOf[Float],
      0.0f,
      1.0f,
      0.0f
