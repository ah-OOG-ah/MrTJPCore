...
class WorldGenClusterizer extends TWorldGenerator {
  ...
  override def generate(w: World, rand: Random, x: Int, y: Int, z: Int) =
    if (clusterSize < 4) generateSmall(w, rand, x, y, z)
    else generateNormal(w, rand, x, y, z)
  ...
  def generateNormal(
    ...
    for (i <- 0 until clusterSize) {
      ...
      val hMod = ((MathHelper.sin(
        (i * Math.PI.toFloat) / clusterSize
      ) + 1f) * size + 1f) * 0.5f
      val vMod = ((MathHelper.sin(
        (i * Math.PI.toFloat) / clusterSize
      ) + 1f) * size + 1f) * 0.5f
