...
class PositionChangeToAction extends ParticleAction {
  ...
  override def operate(p: CoreParticle, time: Double) {
    ...
    if (time < duration) {
      ...
      val speed =
        dpos.copy.multiply(1 / (duration - time)).multiply(deltaTime(time))
  ...
  override def copy =
    ParticleAction.moveTo(target.x, target.y, target.z, duration)
}
...
class PositionChangeForAction extends ParticleAction {
  ...
  override def operate(p: CoreParticle, time: Double) {
    val pp = p.asInstanceOf[TPositionedParticle]
    if (time < duration)
      pp.setPos(pp.position.add(delta.copy.multiply(deltaTime(time))))
    else isFinished = true
  }
  ...
  override def copy =
    ParticleAction.moveFor(delta.x, delta.y, delta.z, duration)
}
