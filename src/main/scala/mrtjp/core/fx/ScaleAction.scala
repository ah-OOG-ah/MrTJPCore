...
class ScaleToAction extends ParticleAction {
  ...
  override def operate(p: CoreParticle, time: Double) {
    ...
    if (time < duration) {
      ...
      val speed =
        dscale.copy.multiply(1 / (duration - time)).multiply(deltaTime(time))
      s.scale.add(speed)
  ...
  override def copy =
    ParticleAction.scaleTo(target.x, target.y, target.z, duration)
}
...
class ScaleForAction extends ParticleAction {
  ...
  override def copy =
    ParticleAction.scaleFor(delta.x, delta.y, delta.z, duration)
}
