...
//    var updateZ = 0.0
  ...
  private def calculateLengthAndRotation() {
    ...
    rotationYaw =
      (math.atan2(deltaX, deltaZ) * 180.0d / 3.141592653589793d).toFloat
    rotationPitch =
      (math.atan2(deltaY, hDist) * 180.0d / 3.141592653589793d).toFloat
  }
...
//    }
  ...
  override def renderParticle(
    ...
    val rot =
      worldObj.provider.getWorldTime % (360.0 / rotateSpeed) * rotateSpeed + rotateSpeed * par2
