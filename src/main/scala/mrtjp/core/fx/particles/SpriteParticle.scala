...
class SpriteParticle(w: World)
  ...
  override def renderParticle(
    ...
    val f11 =
      (prevPosX + (posX - prevPosX) * frame - EntityFX.interpPosX).toFloat
    val f12 =
      (prevPosY + (posY - prevPosY) * frame - EntityFX.interpPosY).toFloat
    val f13 =
      (prevPosZ + (posZ - prevPosZ) * frame - EntityFX.interpPosZ).toFloat
