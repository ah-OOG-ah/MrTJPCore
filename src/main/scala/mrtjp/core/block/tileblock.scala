...
class InstancedBlock(name: String, mat: Material) extends BlockContainer(mat) {
  ...
  def addSingleTile[A <: InstancedBlockTile](t: Class[A]) {
    addTile(t, 0); singleTile = true
  }
  ...
  override def getIcon(side: Int, meta: Int) =
    TileRenderRegistry.getIcon(this, side, meta)
  ...
  override def createTileEntity(world: World, meta: Int) = {
    ...
    try {
      t = if (singleTile) tiles(0).newInstance else tiles(meta).newInstance
  ...
  override def removedByPlayer(
    ...
    if (world.isRemote) true
      ...
      if (
        b.canHarvestBlock(player, md) && !player.capabilities.isCreativeMode
      ) {
        val il = getDrops(
          world,
          x,
  ...
  def postBlockSetup(
    ...
    w.getTileEntity(x, y, z) match {
      case t: InstancedBlockTile =>
        t.onBlockPlaced(side, meta, player, stack, hit)
      case _ =>
    }
  }
  ...
  override def updateTick(w: World, x: Int, y: Int, z: Int, rand: Random) =
    w.getTileEntity(x, y, z) match {
