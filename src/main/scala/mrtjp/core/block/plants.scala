...
trait TPlantBlock extends InstancedBlock with IPlantable with IGrowable {
  ...
  def canStay(w: World, x: Int, y: Int, z: Int) =
  ...
  override def func_149852_a(w: World, rand: Random, x: Int, y: Int, z: Int) =
    false
...
trait TItemSeed extends Item with IPlantable {
  override def onItemUse(
    ...
    if (super.onItemUse(item, player, w, x, y, z, side, hitX, hitY, hitZ))
      return true
    if (side != 1) return false

    if (
      player.canPlayerEdit(x, y, z, side, item) && player.canPlayerEdit(
        x,
        y + 1,
        z,
        side,
      ...
      if (
        w.getBlock(x, y, z)
          .canSustainPlant(w, x, y, z, ForgeDirection.UP, this) && w.isAirBlock(
          x,
          y + 1,
          z
        )
  ...
  override def getPlant(w: IBlockAccess, x: Int, y: Int, z: Int) =
    getPlantBlock._1
  override def getPlantMetadata(w: IBlockAccess, x: Int, y: Int, z: Int) =
    getPlantBlock._2
