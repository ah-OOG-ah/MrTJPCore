...
object ItemKey {
  def get(
      stack: ItemStack
  ): ItemKey = // TODO remove sanity check, should be done externally
    {
      if (stack == null) null
...
class ItemKey(val item: Item, val itemDamage: Int, val tag: NBTTagCompound)
    extends Ordered[ItemKey] {
  lazy val testStack = makeStack(0)
  lazy val itemID = Item.getIdFromItem(item)

  private val hash =
    itemID * 1000001 * itemDamage + (if (tag != null) tag.hashCode else 0)
...
object ItemKeyStack {
  ...
  def get(
      stack: ItemStack
  ): ItemKeyStack = // TODO remove sanity check, should be done externally
    {
      if (stack == null) null
...
class ItemKeyStack(val key: ItemKey, var stackSize: Int)
    extends Ordered[ItemKeyStack] {
...
object ItemKeyConversions {
  ...
  implicit def stackToIKS(stack: ItemStack): ItemKeyStack =
