...
/** Base button class with position and width/height. Doesnt render anything,
...
trait TButtonText extends ButtonNode {
  ...
  abstract override def drawButton(mouseover: Boolean) {
    ...
    GuiDraw.drawStringC(
      text,
      position.x + size.width / 2,
      position.y + (size.height - 8) / 2,
      if (mouseover) 0xffffffa0 else 0xffe0e0e0
    )
    GL11.glColor4f(1, 1, 1, 1)
  }
}
