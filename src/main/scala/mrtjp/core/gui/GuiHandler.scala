...
object GuiHandler {
  ...
  /** Called serverside to open a syncronized container GUI
    ...
    *   Serverside player
    * @param cont
    *   Serverside container
    * @param guiID
    *   Gui ID defined in GuiIDs object
    * @param dataWrite
    *   Partial function that adds data to a packet
    */
  def openSMPContainer(
      player1: EntityPlayer,
      cont: Container,
      guiID: Int,
      dataWrite: MCDataOutput => Unit
  ) {
  ...
  /** Called client side upon receiving a request to open a gui. Server requests
    * client to do so with the above method.
    * @param windowID
    *   The window ID the server defined to client
    * @param gui
    *   The gui created by the client
    */
  ...
  /** Internally called by the client when it receives a gui request from the
    ...
    * @param data
    *   Raw data built by the server, includes windowID as byte and guiID as
    *   short, as well as custom gui data.
    */
  @SideOnly(Side.CLIENT)
...
trait TGuiBuilder {
  ...
  def open(player: EntityPlayer, cont: Container) {
    open(player, cont, { packet => })
  }
  def open(
      player: EntityPlayer,
      cont: Container,
