...
class PanNode extends TNode {
  ...
  override def frameUpdate_Impl(mouse: Point, rframe: Float) {
    ...
    if (mouseDownRight || mouseDownBelow) {
      ...
      val l = 0 + clampSlack
      val lc =
        if (cFrame.size.width > size.width) cFrame.origin.x max l
        else cFrame.origin.x min l
      val ld = l - lc
      val r = size.width - clampSlack
      val rc =
        if (cFrame.size.width > size.width) cFrame.maxX min r
        else cFrame.maxX max r
      val rd = r - rc

      // y disp
      val t = 0 + clampSlack
      val tc =
        if (cFrame.size.height > size.height) cFrame.origin.y max t
        else cFrame.origin.y min t
      val td = t - tc
      val b = size.height - clampSlack
      val bc =
        if (cFrame.size.height > size.height) cFrame.maxY min b
        else cFrame.maxY max b
      val bd = b - bc
  ...
  override def traceHit(absPoint: Point) = raytestMode match {
    ...
    case 1 =>
      getScrollBarRight.contains(parent.convertPointFromScreen(absPoint))
    case 2 =>
      getScrollBarBelow.contains(parent.convertPointFromScreen(absPoint))
