...
trait TNode extends Gui {
  ...
  def convertPointToScreen(p: Point) =
    getRoot.position + convertPointTo(p, getRoot)
  def convertPointFromScreen(p: Point) =
    convertPointFrom(p, getRoot) - getRoot.position
  ...
  def convertPointTo(p: Point, to: TNode): Point = {
    ...
    def convertDown(low: TNode, high: TNode) = fold(low, high) {
      _ - _.position
    }
    ...
    if (this == to) p
      ...
      throw new Exception(
        "Attempted to convert points between unrelated nodes."
      )
  }
  def convertPointFrom(p: Point, from: TNode): Point =
    from.convertPointTo(p, this)

  def convertRectTo(r: Rect, to: TNode): Rect =
    Rect(convertPointTo(r.origin, to), r.size)
  def convertRectFrom(r: Rect, from: TNode): Rect =
    Rect(convertPointFrom(r.origin, from), r.size)
  def convertRectToScreen(r: Rect): Rect =
    Rect(convertPointToScreen(r.origin), r.size)
  def convertRectFromScreen(r: Rect): Rect =
    Rect(convertPointFromScreen(r.origin), r.size)

  def calculateChildrenFrame: Rect = {
    val rect =
      if (children.isEmpty) Rect.zeroRect
      else
        children
          .filterNot(_.hidden)
          .map(_.calculateAccumulatedFrame)
  ...
  def subTree(activeOnly: Boolean = false) = {
    ...
    def gather(children: Seq[TNode]) {
      val ac =
        if (activeOnly)
          children.filter(c => !c.hidden && c.userInteractionEnabled)
        else children
      s ++= ac
  ...
  private final def operate2(
      consumed: Boolean
  )(self: (Boolean) => Boolean)(sub: (TNode, Boolean) => Boolean) = {
    familyByZ.reverse.foldLeft(consumed)((c, w) =>
      (if (w == this) self(c) else sub(w, c)) || c
    )
  }

  protected[gui] def mouseClicked(
      p: Point,
      button: Int,
      consumed: Boolean
  ): Boolean = {
    ...
    val dp = p - position
    operate2(consumed) { mouseClicked_Impl(p, button, _) } {
      _.mouseClicked(dp, button, _)
    }
  }

  protected[gui] def mouseReleased(
      p: Point,
      button: Int,
      consumed: Boolean
  ): Boolean = {
    if (hidden || !userInteractionEnabled) return false
    val dp = p - position
    operate2(consumed) { mouseReleased_Impl(p, button, _) } {
      _.mouseReleased(dp, button, _)
    }
  }

  protected[gui] def mouseDragged(
      p: Point,
      button: Int,
      time: Long,
      consumed: Boolean
    ...
    if (hidden || !userInteractionEnabled) return false
    val dp = p - position
    operate2(consumed) { mouseDragged_Impl(p, button, time, _) } {
      _.mouseDragged(dp, button, time, _)
    }
  }

  protected[gui] def mouseScrolled(
      p: Point,
      dir: Int,
      consumed: Boolean
  ): Boolean = {
    if (hidden || !userInteractionEnabled) return false
    val dp = p - position
    operate2(consumed) { mouseScrolled_Impl(p, dir, _) } {
      _.mouseScrolled(dp, dir, _)
    }
  }

  protected[gui] def keyPressed(
      ch: Char,
      keycode: Int,
      consumed: Boolean
  ): Boolean = {
    if (hidden || !userInteractionEnabled) return false
    operate2(consumed) { keyPressed_Impl(ch, keycode, _) } {
      _.keyPressed(ch, keycode, _)
    }
  }
  ...
  /** Called constantly while the mouse is held down.
    ...
    *   If this node has consumed this event.
    */
  def mouseDragged_Impl(p: Point, button: Int, time: Long, consumed: Boolean) =
    false
