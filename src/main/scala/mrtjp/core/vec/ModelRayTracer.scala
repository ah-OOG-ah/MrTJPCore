...
object ModelRayTracer {
  def raytraceModel(
    ...
    def getSide(vec: Vector3) = {
      ...
      Seq((-y1, 0), (y1, 1), (-z1, 2), (z1, 3), (-x1, 4), (x1, 5))
        .maxBy(_._1)
        ._2
    }
    ...
    raytraceModel(start, dir, model) match {
      case Some((dist, tri)) =>
        val side = getSide(
          tri.normal.copy.add(
            start.copy.add(dir).multiply(dist).multiply(0.001)
          )
        )
        val mop = new MovingObjectPosition(
          x.toInt,
          y.toInt,
          z.toInt,
          side,
          calcPlayerHit(
            new Vector3(x, y, z),
            from.copy.add(dir.copy.multiply(dist))
          ).toVec3D
        )
        mop.subHit = 0
        mop
  ...
  private def raytraceModel(from: Vector3, dir: Vector3, model: CCModel) = {
    val faces =
      for (i <- 0 until model.getVertices.length by 4)
        yield Quad(
          model.verts(i).vec,
          model.verts(i + 1).vec,
          model.verts(i + 2).vec,
    ...
    tfaces.foreach(t =>
      mt(from, dir, t.v0, t.v1, t.v2) match {
        case Some(dist) =>
