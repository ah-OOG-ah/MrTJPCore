...
object InvWrapper {
  ...
  def areItemsStackable(stack1: ItemStack, stack2: ItemStack): Boolean = {
    stack1 == null || stack2 == null || areItemsSame(
      stack1,
      stack2
  ...
  def areItemsSame(stack1: ItemStack, stack2: ItemStack): Boolean = {
    ...
    stack1.getItem == stack2.getItem && stack2.getItemDamage == stack1.getItemDamage && ItemStack
      .areItemStackTagsEqual(stack2, stack1)
  }
...
class HashableLargeChest(
    name: String,
    val inv1: IInventory,
    val inv2: IInventory
) extends InventoryLargeChest(name, inv1, inv2) {
...
abstract class InvWrapper(val inv: IInventory) {
  ...
  /** Get a count for how many items of this type can be shoved into the
    ...
    *
    * @param item
    *   The item to count free space for. Not manipulated in any way.
    * @return
    *   The number of those items this inventory can still take.
    */
  def getSpaceForItem(item: ItemKey): Int

  /** Check if at least one of this item can fit. Failfast for getSpaceForItem
    * @param item
    *   The item to count free space for. Not manipulated in any way.
    * @return
    *   True if one of these items can fit
    */
  ...
  /** Counts how many of those items this inventory contains.
    *
    * @param item
    *   The item to count. Not manipulated in any way.
    * @return
    *   The number of those items this inventory contains.
    */
  ...
  /** Returns if the given item is in the inventory somewhere. Failfast of
    ...
    *
    * @param item
    *   the item. Not manipulated in any way.
    * @return
    */
  ...
  /** Inject the ItemStack into the inventory, starting with merging, then to
    ...
    *
    * @param item
    *   The item to try and merge. Not manipulated in any way.
    * @param toAdd
    *   Amount to try to add.
    * @return
    *   The number of items that were merged in, between 0 and toAdd.
    */
  ...
  /** Extract the item a specified number of times.
    *
    * @param item
    *   Item to extract from inventory. Not manipulated in any way.
    * @param toExtract
    *   Amount to try to extract.
    * @return
    *   Amount extracted, between 0 and toExtract.
    */
  ...
  protected def canInsertItem(slot: Int, item: ItemStack): Boolean = {
    ...
    if (side < 0) inv.isItemValidForSlot(slot, item)
    else sidedInv.canInsertItem(slot, item, side)
  }

  protected def canExtractItem(slot: Int, item: ItemStack): Boolean = {
    if (internalMode) return true
    if (side < 0) inv.isItemValidForSlot(slot, item)
    else sidedInv.canExtractItem(slot, item, side)
  }
}
...
trait TDefWrapHandler extends InvWrapper {
  override def getSpaceForItem(item: ItemKey): Int = {
    ...
    val slotStackLimit =
      math.min(inv.getInventoryStackLimit, item.getMaxStackSize)
    for (slot <- slots) {
      val s = inv.getStackInSlot(slot)
      if (canInsertItem(slot, item2)) {
        if (s == null) space += slotStackLimit
        else if (InvWrapper.areItemsStackable(s, item2))
          space += slotStackLimit - s.stackSize
      }
    }
    space
  }
  ...
  override def hasSpaceForItem(item: ItemKey): Boolean = {
    val item2 = item.testStack
    val slotStackLimit =
      math.min(inv.getInventoryStackLimit, item2.getMaxStackSize)
    for (slot <- slots) {
      val s = inv.getStackInSlot(slot)
      if (canInsertItem(slot, item2)) {
        if (s == null) return true
        else if (
          InvWrapper.areItemsStackable(
            s,
            item2
          ) && slotStackLimit - s.stackSize > 0
        ) return true
      }
    }
    false
  ...
  override def getItemCount(item: ItemKey) = {
    ...
    for (slot <- slots) {
      ...
      if (inSlot != null && eq.matches(item, ItemKey.get(inSlot))) {
        val toAdd =
          inSlot.stackSize - (if (hidePerSlot || hidePerType && first) 1 else 0)
        first = false
        count += toAdd
      }
    }
  ...
  override def injectItem(item: ItemKey, toAdd: Int): Int = {
    ...
    for (pass <- Seq(0, 1))
      for (slot <- slots) if (canInsertItem(slot, item.testStack)) {
        val inSlot = inv.getStackInSlot(slot)

        if (
          inSlot != null && InvWrapper.areItemsStackable(item.testStack, inSlot)
        ) {
          ...
          val toInsert =
            item.makeStack(math.min(inv.getInventoryStackLimit, itemsLeft))
  ...
  override def extractItem(item: ItemKey, toExtract: Int): Int = {
    ...
    for (slot <- slots) if (canExtractItem(slot, item.testStack)) {
      val inSlot = inv.getStackInSlot(slot)
      if (
        inSlot != null && eq.matches(item, ItemKey.get(inSlot))
      ) // TODO extraction shouldnt rely on eq matches..?
        {
          left -= inv
            .decrStackSize(
              slot,
              math.min(
                left,
                inSlot.stackSize - (if (hidePerSlot || hidePerType && first) 1
                                    else 0)
              )
            )
            .stackSize
    ...
    for (slot <- slots) {
      ...
      if (inSlot != null) {
        ...
        if (!items.keySet.contains(key))
          items += key -> (stackSize - (if (hidePerType) 1 else 0))
...
class VanillaWrapper(inv: IInventory)
    extends InvWrapper(inv)
    with TDefWrapHandler
